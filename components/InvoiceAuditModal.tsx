...
export const InvoiceAuditModal: React.FC<InvoiceAuditModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div
        className="relative w-[95vw] sm:w-full max-w-4xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          ...
          {totalERP === 0 ? (
            <div className="p-4 bg-slate-100/90 border border-slate-300 rounded-2xl text-slate-700 space-y-1.5 opacity-90">
              <div className="flex items-center gap-2 text-slate-800 font-medium text-xs">
                <span className="w-3 h-3 rounded-full bg-slate-400 inline-block" />
                <span>⚪ NO ENCONTRADO EN FROG ERP (Sin Match Registrado)</span>
              </div>
              <p className="text-xs font-normal text-slate-500 leading-relaxed">
                Esta factura por <strong className="text-slate-800 font-black">{formatCurrency(totalSAT)}</strong> existe legalmente ante el SAT, pero <span className="underline">no se encontró ninguna recepción o folio coincidente en el ERP de Grupo MV</span>.
              </p>
            </div>
          ) : esCritico ? (
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <div className="flex items-center gap-2">
                  <img src="/logos/sat.png" alt="SAT" className="h-6 object-contain rounded" />
